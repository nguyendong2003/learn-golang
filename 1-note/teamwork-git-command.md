...
### Câu hỏi 6: Câu lệnh git hàng ngày

```
git checkout feature/add-coupon
git add .
git commit -m "feature: add coupon"
git pull --rebase origin feature/add-coupon    (sau đó resolve conflict nếu có)
git push --force-with-lease origin feature/add-coupon
```
