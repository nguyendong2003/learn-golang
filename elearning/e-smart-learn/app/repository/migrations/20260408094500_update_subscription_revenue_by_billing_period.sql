...
WITH successful_payments AS (
	...
	WHERE p.status = 'succeeded'
	  ...
	  AND (p_from IS NULL OR (p.paid_at AT TIME ZONE 'UTC') >= p_from)
	  AND (p_to IS NULL OR (p.paid_at AT TIME ZONE 'UTC') < p_to)
),
...
WITH successful_payments AS (
	...
	WHERE p.status = 'succeeded'
	  AND p.paid_at IS NOT NULL
	  AND (p.paid_at AT TIME ZONE 'UTC') >= p_from
	  AND (p.paid_at AT TIME ZONE 'UTC') < p_to
),
active_enrollments AS (
	SELECT
		sp.payment_id,
