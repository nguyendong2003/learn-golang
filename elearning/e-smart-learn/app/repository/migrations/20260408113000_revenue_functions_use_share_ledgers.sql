...
-- Course purchase revenue functions (ledger-backed)
...
AS $$
SELECT
	COALESCE(SUM(detail_amount), 0)::BIGINT AS total_amount,
...
-- Subscription revenue functions (ledger-backed)
...
LANGUAGE sql
AS $$
SELECT * FROM get_instructor_subscription_revenue_range(
	p_user_id,
	TIMESTAMPTZ '1970-01-01 00:00:00+00',
	NOW()
);
...
LANGUAGE sql
AS $$
SELECT * FROM get_admin_subscription_revenue_range(
	TIMESTAMPTZ '1970-01-01 00:00:00+00',
	NOW()
);
$$;
...
-- +goose Down
-- +goose StatementBegin
-- Keep latest function definitions on rollback.
SELECT 1;
-- +goose StatementEnd
