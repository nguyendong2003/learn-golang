...
('Go Backend Development Masterclass','Learn scalable backend with Golang','https://example.com/go.jpg','go-backend-development',(SELECT id FROM users WHERE username='instructor1'),600,(SELECT id FROM categories WHERE name='Backend Development'),49.99,4.8,'published',850,'prod_UIFSzDvfuZVoQX','price_1TJey3FGuK3jvcTm9TQiH33x',4999,'usd'),
('React From Zero to Hero','Complete React course with hooks and advanced patterns','https://example.com/react.jpg','react-zero-to-hero',(SELECT id FROM users WHERE username='instructor2'),720,(SELECT id FROM categories WHERE name='Frontend Development'),39.99,4.7,'published',640,'prod_UIFTWMcY0uk74Q','price_1TJeybFGuK3jvcTmYRf8MroC',3999,'usd'),
('Docker & Kubernetes Practical Guide','Learn containerization and orchestration','https://example.com/docker.jpg','docker-kubernetes-guide',(SELECT id FROM users WHERE username='instructor3'),540,(SELECT id FROM categories WHERE name='DevOps'),59.99,4.9,'published',1200,'prod_UIFTAJVOg078el','price_1TJeyyFGuK3jvcTmOD1PZ8kF',5999,'usd'),
('Microservices with Go','Build microservices using Go and gRPC','https://example.com/microservices.jpg','microservices-go',(SELECT id FROM users WHERE username='instructor1'),650,(SELECT id FROM categories WHERE name='Backend Development'),54.99,4.8,'published',430,'prod_UIFUvTZgMbuAni','price_1TJez9FGuK3jvcTm9Y6zgBbW',5499,'usd'),
('Advanced Python for Data Science','Master machine learning with Python and NumPy','https://example.com/python-ds.jpg','python-data-science',(SELECT id FROM users WHERE username='instructor4'),580,(SELECT id FROM categories WHERE name='Data Science'),44.99,4.6,'published',320,'prod_UIFUiZWS7nwPVy','price_1TJezGFGuK3jvcTmgWkCZQgi',4499,'usd'),
('Cloud Architecture on AWS','Design and deploy scalable cloud solutions','https://example.com/aws.jpg','aws-cloud-architecture',(SELECT id FROM users WHERE username='instructor5'),500,(SELECT id FROM categories WHERE name='Cloud Computing'),64.99,4.7,'published',520,'prod_UIFU6j58nmQwII','price_1TJezMFGuK3jvcTmUpAPWrnP',6499,'usd');
...
('Premium Monthly','Premium Plan - Subscription hàng tháng',CAST('monthly' AS billing_cycle_enum),10,'prod_UIF42xDCDvNkfu','price_1TJeajFGuK3jvcTmWfqntzAX','usd', true),
('Premium Yearly','Premium Plan - Subscription hàng năm',CAST('yearly' AS billing_cycle_enum),100,'prod_UIF4gwLJsrGv6Z','price_1TJeZtFGuK3jvcTmW5f0YJft','usd', true);
...
('ABCDE1','percent',20,200,0,true,'2026-12-31 23:59:59','G8K1SKI7','promo_1TJeV2FGuK3jvcTmbBowp4qJ'),
('SAVE30','percent',30,10000,0,true,'2026-12-31 23:59:59','bqML7U1h','promo_1TJeUFFGuK3jvcTmSBjDMNSg'),
('SUMMER2000','amount',2000,100,0,true,'2026-12-31 23:59:59','jLbW5bNJ','promo_1TJeSEFGuK3jvcTmMoygHLfU');
